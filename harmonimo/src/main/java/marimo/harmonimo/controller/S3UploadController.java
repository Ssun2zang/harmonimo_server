...
public class S3UploadController {
    ...
    @ResponseBody   // Long 타입을 리턴하고 싶은 경우 붙여야 함 (Long - 객체)
    @PostMapping(value="/uploadNewRec",consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<UserRecDTO> uploadRec(HttpServletRequest request, @RequestParam(value="file") MultipartFile image) throws IOException {
        UserRecDTO userRecDTO = new UserRecDTO();
        System.out.println("Upload Rec");
        System.out.println(image);
        System.out.println("------------------------------------------------------");
        if(!image.isEmpty()) {
            String storedFileName = s3Uploader.upload(image,"records");
            userRecDTO.setUploadRec(storedFileName);
        }
